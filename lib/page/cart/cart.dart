...
import 'package:redofyp/page/cart/serviceCheckout.dart';
                          ...
                          GestureDetector(
                            onTap: () async {
                              ...
                              if (selectedTime != null) {
                                // Update Firestore for this cart item
                                try {
                                  await FirebaseFirestore.instance
                                      .collection('users')
                                      .doc(widget.userId)
                                      .collection('cartItems')
                                      .doc(item['id'])
                                      .update({'serviceTime': selectedTime});

                                  // Update local state for immediate UI update
                                  setState(() {
                                    item['serviceTime'] = selectedTime;
                                  });
                                } catch (e) {
                                  print('Error updating service time: $e');
                                  _showErrorDialog('Failed to update service time. Please try again.');
                                }
                              }
                            },
                            child: Text(
                              item['serviceTime'] ?? 'N/A',
                              style: TextStyle(
                                fontSize: 14,
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  // Check if the selected items belong to different categories
                  String? firstCategory = selectedItems.isNotEmpty ? selectedItems[0]['listingType'] : null;
                  bool hasDifferentCategories = selectedItems.any((item) => item['listingType'] != firstCategory);

                  if (hasDifferentCategories) {
                    // Show error dialog if items are from different categories
                    _showErrorDialog("You can only checkout items from the same category!");
                    return;
                  }
